...
{
    //debug/ <summary>
    //debug/ Allows grabbing and throwing of objects with the DistanceGrabbable component on them.
    //debug/ </summary>
    [RequireComponent(typeof(Rigidbody))]
    public class DistanceGrabber : OVRGrabber
    {
        //debug Radius of sphere used in spherecast from hand along forward ray to find target object.
        [SerializeField]
        float m_spherecastRadius = 0;

        //debug Distance below which no-snap objects won't be teleported, but will instead be left
        //debug where they are in relation to the hand.
        [SerializeField]
        ...
        //debug Public to allow changing in demo.
        [SerializeField]
        public bool m_preventGrabThroughWalls;

        [SerializeField]
        float m_objectPullVelocity = 10.0f;
        float m_objectPullMaxRotationRate = 360.0f; //debug max rotation rate in degrees per second

        bool m_movingObjectToHand = false;

        //debug Objects can be distance grabbed up to this distance from the hand.
        [SerializeField]
        float m_maxGrabDistance;

        //debug Only allow grabbing objects in this layer.
		//debug NOTE: you can use the value -1 to attempt to grab everything.
        [SerializeField]
        ...
        //debug Tracked separately from m_target, because we support child colliders of a DistanceGrabbable.
        ...
        protected override void Start()
        {
            ...
            //debug Basic hack to guess at max grab distance based on player size.
            //debug Note that there's no major downside to making this value too high, as objects
            //debug outside the player's grabbable trigger volume will not be eligible targets regardless.
        ...
        protected override void GrabBegin()
        {
            ...
            if (closestGrabbable != null)
            {
                ...
                //debug If it's within a certain distance respect the no-snap.
                ...
                {
                    //debug Set up offsets for grabbed object desired position relative to hand.
        ...
        protected bool FindTarget(out DistanceGrabbable dgOut, out Collider collOut)
        {
            ...
            //debug First test for objects within the grab volume, if we're using those.
            //debug (Some usage of DistanceGrabber will not use grab volumes, and will only 
            //debug use spherecasts, and that's supported.)
            foreach (OVRGrabbable cg in m_grabCandidates.Keys)
            {
                ...
                for (int j = 0; j < grabbable.grabPoints.Length; ++j)
                {
                    ...
                    //debug Store the closest grabbable
                    ...
                    if (grabbableMagSq < closestMagSq)
                    {
                        ...
                        {
                            //debug NOTE: if this raycast fails, ideally we'd try other rays near the edges of the object, especially for large objects.
                            //debug NOTE 2: todo optimization: sort the objects before performing any raycasts.
                            Ray ray = new Ray();
        ...
        protected bool FindTargetWithSpherecast(out DistanceGrabbable dgOut, out Collider collOut)
        {
			...
			//debug If no objects in grab volume, raycast.
			//debug Potential optimization: 
			//debug In DistanceGrabbable.RefreshCrosshairs, we could move the object between collision layers.
			//debug If it's in range, it would move into the layer DistanceGrabber.m_grabObjectsInLayer,
			//debug and if out of range, into another layer so it's ignored by DistanceGrabber's SphereCast.
			//debug However, we're limiting the SphereCast by m_maxGrabDistance, so the optimization doesn't seem
			//debug essential.
            ...
            if (Physics.SphereCast(ray, m_spherecastRadius, out hitInfo, m_maxGrabDistance, layer))
            {
                ...
                {
                    //debug Found a valid hit. Now test to see if it's blocked by collision.
                    RaycastHit obstructionHitInfo;
        ...
        //debug Just here to allow calling of a protected member function.
